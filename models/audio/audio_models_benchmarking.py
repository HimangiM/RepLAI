...
__all__ = [
    ...
    'Wavegram_Cnn14', 'Wavegram_Logmel_Cnn14'
]
...
class Cnn6(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        for k in state_dict:
            if 'spectrogram_extractor' in k or \
                    'logmel_extractor' in k or \
                    'spec_augmenter' in k or \
                    'bn0' in k:
                del state_dict[k]
...
class Wavegram_Cnn14(nn.Module):
    def __init__(self, classes_num=527, pretrained=False):
        ...
        if pretrained:
            import urllib.request
            ckpt_fn = 'models/pretrained/audio_benchmarking/Wavegram_Cnn14_mAP_0.389.pth'
            url = 'https://zenodo.org/record/3987831/files/Wavegram_Cnn14_mAP%3D0.389.pth'
            if not os.path.isfile(ckpt_fn):
                os.makedirs(os.path.dirname(ckpt_fn), exist_ok=True)
                urllib.request.urlretrieve(url, ckpt_fn)
            self.load_state_dict(torch.load(ckpt_fn, map_location='cpu'))
