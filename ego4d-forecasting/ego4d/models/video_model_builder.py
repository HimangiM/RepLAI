...
class R2Plus1D(nn.Module):
    ...
    def _construct_network(self, cfg, with_head=True):
        ...
        width = cfg.R2PLUS1D.WIDTH

        self.conv1 = nn.Sequential(
            nn.Conv3d(dim_in, width, kernel_size=(3, 7, 7), padding=(1, 3, 3), stride=(1, 2, 2), bias=False),
            self.norm_module(width),
        ...
        strides = {
            10: [(1, 1, 1), (2, 2, 2), (2, 2, 2), (2, 2, 2)],
            18: [(1, 1, 1), (2, 2, 2), (2, 2, 2), (2, 2, 2)],
            34: [(1, 1, 1), (2, 2, 2), (2, 2, 2), (2, 2, 2)],
        }[cfg.R2PLUS1D.DEPTH]

        def _build_stage(in_dim, stage_dim, blocks, stride):
            modules = []
            for i in range(blocks):
                din = in_dim if i == 0 else stage_dim
                dout = stage_dim
                s = stride if i == 0 else (1, 1, 1)
                modules.append(BasicR2P1DBlock(din, dout, stride=s))
            return nn.Sequential(*modules) if len(modules) > 1 else modules[0]

        self.conv2x = _build_stage(width*1, width*1, num_blocks[0], strides[0])
        self.conv3x = _build_stage(width*1, width*2, num_blocks[1], strides[1])
        self.conv4x = _build_stage(width*2, width*4, num_blocks[2], strides[2])
        self.conv5x = _build_stage(width*4, width*8, num_blocks[3], strides[3])
        ...
        if self.enable_detection:
            ...
            head = head_helper.ResNetBasicHead(
                ...
                pool_size=[
                    [
                        cfg.DATA.NUM_FRAMES // 8 // pool_size[0][0],
                        cfg.DATA.CROP_SIZE // 32 // pool_size[0][1],
                        cfg.DATA.CROP_SIZE // 32 // pool_size[0][2],
                    ]
                ],
