...
def clip_recognition_dataset(
            ...
            untrimmed_clip_annotations.append(
                (
                    os.path.join(video_path_prefix, f'{entry["clip_uid"]}.mp4'),
                    {
                        ...
                        "verb_label": entry['verb_label'] if 'verb_label' in entry else -1,
                        "action_idx": entry['action_idx'] if 'action_idx' in entry else -1,
                    },
                )
            )
    else:
    ...
    available_clips = [clip for clip in untrimmed_clip_annotations if g_pathmgr.isfile(clip[0])]
